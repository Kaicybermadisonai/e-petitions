...
class CookieManager {
  constructor(root) {
    ...
    if (!this.preferencesSet) {
      this.banner.show();
    } else if (this.allowAnalyticsCookies) {
  ...
  writeAnalyticsScriptTag() {
    ...
    googleTag.async = 1;
    googleTag.src = `https://www.googletagmanager.com/gtm.js?id=${this.googleTagManagerId}`;

    var scriptTag = document.getElementsByTagName('script')[0];
    scriptTag.parentNode.insertBefore(googleTag, scriptTag);
  }
